...
$bulan = filter_var($_GET['bulan'] ?? '', FILTER_VALIDATE_INT, ['options' => ['min_range' => 1, 'max_range' => 12]]) ?: '';
$tahun = filter_var($_GET['tahun'] ?? '', FILTER_VALIDATE_INT, ['options' => ['min_range' => 2024, 'max_range' => (int) date('Y')]]) ?: '';
...
if ($bulan && $tahun) {
    $where[] = 'MONTH(tamu.tanggal_kunjungan) = ? AND YEAR(tamu.tanggal_kunjungan) = ?';
    $types .= 'ii';
    $params[] = $bulan;
    $params[] = $tahun;
} else {
    $where[] = 'tamu.tanggal_kunjungan BETWEEN ? AND ?';
    $types .= 'ss';
    $params[] = $from;
    $params[] = $to;
}
...
$statsSql = "
    SELECT
        COUNT(*) as total,
        SUM(CASE WHEN tamu.status='diterima' THEN 1 ELSE 0 END) as disetujui,
...
$s = mysqli_fetch_assoc(mysqli_stmt_get_result($statsStmt));
...
<div class="container-fluid py-4">
        ...
        <form method="GET" class="row g-3 align-items-end">
            ...
            <div class="col-md-2">
                ...
                <select name="bulan" class="form-control form-control-sm">
                    ...
                    <?php foreach ($months as $idx => $m): $val = $idx + 1; ?>
                        <option value="<?= $val; ?>" <?= (int) $bulan === $val ? 'selected' : ''; ?>><?= e($m); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="col-md-1">
    ...
    <div class="row mb-4">
        ...
        $cards = [
            ...
            ['Disetujui', $s['disetujui'] ?? 0, 'text-success'],
        ...
        <div class="table-responsive">
            <table id="tabelLaporan" class="table admin-table align-items-center mb-0">
                ...
                <tbody>
                    ...
                    <?php endwhile; mysqli_stmt_close($stmt); mysqli_stmt_close($statsStmt); ?>
                </tbody>
            </table>
        </div>
    </div>
